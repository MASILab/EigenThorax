...
UNION_AVERAGE_IMG=${OUT_ROOT}/average/union.nii.gz

echo "#####################"
echo "# Step.0 Downsample the images"
DOWNSAMPLE_FOLDER=${OUT_ROOT}/preprocess/resample
DOWNSAMPLE_REF_IMG=/nfs/masi/xuk9/SPORE/clustering/pca/2020311_10_dataset/average/union.nii.gz
mkdir -p ${DOWNSAMPLE_FOLDER}
mkdir -p ${OUT_FOLDER}
#set -o xtrace
#${PYTHON_ENV} ${SRC_ROOT}/src/run_resample.py \
#  --config ${CONFIG} \
#  --in-folder ${IN_FOLDER} \
#  --out-folder ${DOWNSAMPLE_FOLDER} \
#  --ref-img ${DOWNSAMPLE_REF_IMG}
#set +o xtrace
