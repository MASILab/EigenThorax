...
int main (int argc, char * const argv[]) {
    ...
    for(int level=0;level<args.levels;level++){
        ...
        cout<<"M"<<flush;
        gettimeofday(&time1, NULL);
        // This is the F: Caculate similarity-term using dense sampling
        ...
        // This is the B: Extract minimum-spaning tree
        primsGraph(im1b,ordered,parents,edgemst,step1,m,n,o);

        // This is the C: Infer regularisation using message-passing
    ...
    writeOutput(flow,outputflow.c_str(),sz1*6);
    warpAffine(warped1,im1,im1b,X,ux,vx,wx);
